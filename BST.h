...
void BST::insert_node(int value) // Finish insert_node function
{
  ...
  if (new_node) // If create new node
  {
      if (size == 0)
      {
        rootPtr = new_node;
      ...
      else 
      {
        ...
        while (inserted == 0) // everytime the function is called inserted == 0
        {
              ...
              if (t->get_value() >= value) 
              {
                ...
                else // Left side is NULL, available, insert
                {
                  t->set_left(new_node);
                  inserted = 1; // Stop the search
                }
